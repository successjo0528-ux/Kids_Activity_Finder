...
flowchart TD
    ...
    C --> D[Auto-Purge 마감 데이터 자동 배제]
    D --> E[(3중 동기화: data/ web/ root activities.json)]
    E --> F[verify_all.py 9단계 전수 무결성 검증]
    F -->|검증 PASS| G{GitHub Pages 배포 파이프라인}
    G -->|옵션 1: Actions 배포| H[deploy_pages.yml]
    G -->|옵션 2: Branch 배포| I[_config.yml & .nojekyll 보호막]
    H --> J[🚀 GitHub Pages 라이브 서비스]
    I --> J
```
...
## 4. GitHub Pages 듀얼 배포 및 빌드 에러 방지 아키텍처

| 항목 | 설정 파일 | 역할 및 메커니즘 |
| :--- | :--- | :--- |
| **Jekyll 빌드 방어막** | `_config.yml` | `exclude` 목록에 `*.py`, `core`, `scrapers`, `__pycache__`, `*.md` 등을 등록하여 Jekyll이 파이썬 스크립트나 한글 문서를 빌드하려다 발생하는 에러를 100% 차단 |
| **Jekyll 바이패스 플래그** | `.nojekyll`, `web/.nojekyll` | 정적 웹 리소스(HTML, JS, CSS, JSON)를 템플릿 변환 없이 원본 그대로 서빙하도록 지시하는 표준 0바이트 플래그 |
| **Actions 자동 배포 파이프라인** | `.github/workflows/deploy_pages.yml` | `actions/deploy-pages@v4`를 통해 Jekyll을 거치지 않는 초고속 정적 배포 수행 |
| **듀얼 배포 호환성** | `Deploy from a branch` & `GitHub Actions` | 저장소 설정에서 어느 배포 모드를 선택하더라도 오류 없이 100% 정상 배포 지원 |
...
| **[검증 4]** | URL 실시간 생존 감사 (URL Health Check) | 전체 수집 데이터의 고유 URL(120여 개)에 대해 실시간 HTTP 요청을 보내 404 Not Found 에러 0건 검증 | ✅ PASS |
| **[검증 5]** | 중복 제거 에이전트 감사 | 잔여 중복 카드 0건 (100% 무결점 정제) 검증 | ✅ PASS |
| **[검증 6]** | 날짜 및 상태 정합성 | YYYY-MM-DD 정규식 일치 및 접수예정 D-Day 일치성 전수 검사 | ✅ PASS |
| **[검증 7]** | 모바일 PWA 파일 검사 | `index.html`, `style.css`, `app.js`, `manifest.json`, `activities.json` 파일 존재 및 크기 검사 | ✅ PASS |
| **[검증 8]** | 대시보드 등록 검사 | `Tool_Dashboard/programs.json` 등록 상태 검사 | ✅ PASS |
| **[검증 9]** | GitHub Pages 배포 무결성 | `.nojekyll`, `_config.yml`, Actions 워크플로우 5종 정합성 전수 검사 | ✅ PASS |
