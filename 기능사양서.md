...
## 7. 자동화 스케줄 및 운영 주기
- **크롤링 실행 메커니즘:** cron-job.org 외부 Webhook 정시 트리거 (`workflow_dispatch`)
- **실행 주기:** 매일 정시 (지연 없는 즉시 클라우드 가동 및 배포)
- **동시성 안전장치:** `concurrency: kids-activity-crawler` (중복 실행 방지 및 자동 rebase 병합)
